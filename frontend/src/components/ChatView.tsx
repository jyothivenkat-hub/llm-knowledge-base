...
import { Send, Bot, User, Sparkles, Database, ArrowRight, Network } from 'lucide-react';
...
export default function ChatView({ state, initialQuery = '' }: { state: AppState, initialQuery?: string }) {
  ...
  const handleSend = async () => {
    ...
    try {
      const result = await askQuestion(userMessage, (msg) => {
        // Could show progress messages
      });
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div ref={scrollRef} className="flex-1 overflow-auto p-8 space-y-8">
      ...
      <div className="p-8 border-t border-[#a2a9b1] bg-[#f8f9fa]">
        <div className="max-w-3xl mx-auto relative">
          <input
            ...
            className="w-full bg-white border border-[#a2a9b1] rounded py-4 pl-6 pr-16 text-[#202122] focus:outline-none focus:border-[#3366cc] transition-all shadow-sm"
          />
          <button
            onClick={handleSend}
            disabled={!input.trim() || isLoading}
            className="absolute right-3 top-1/2 -translate-y-1/2 p-2.5 bg-[#3366cc] hover:bg-[#2a4b8d] disabled:opacity-50 text-white rounded transition-all"
          >
          ...
          <div className="absolute -top-12 left-0 flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[#a2a9b1] rounded-full text-[10px] font-bold text-[#54595d] hover:text-[#202122] transition-all shadow-sm"
            >
              <Sparkles className="w-3 h-3 text-[#3366cc]" />
              Summarize all claims
            </button>
            <button
