            ...
            <div className="p-8 flex-1">
              <div className="flex items-center justify-between mb-8">
                ...
                <div className={cn(
                  ...
                  idea.difficulty === 'easy' ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
              ...
              <h2 className="text-2xl font-serif font-bold text-[#202122] mb-4 group-hover:text-[#3366cc] transition-colors">{idea.title}</h2>
              ...
              {idea.backingClaims.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {idea.backingClaims.map(claimId => {
                      const claim = state.claims.find(c => c.id === claimId);
                      return (
                        <div key={claimId} className="px-3 py-1.5 bg-white border border-[#a2a9b1] rounded text-[10px] text-[#54595d] hover:text-[#202122] hover:border-[#3366cc] transition-all cursor-help">
                          {claim ? claim.text.substring(0, 40) + '...' : claimId}
                        </div>
                      );
                    })}
                  </div>
